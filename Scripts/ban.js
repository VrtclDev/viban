...
const pendingBlocks = async (a, c="-1") => {
  return nodeAPI({action:"accounts_pending", accounts:[a], count=c})
} 
...
const withdraw = async (priv, to, amnt, raw=false, rep="ban_1ka1ium4pfue3uxtntqsrib8mumxgazsjf58gidh1xeo5te3whsq8z476goo") => {
  ...
  block.balance = bal
  var work = await genWork(block.previous)
  block.work = work.response.work
  block.signature = signHash(priv, hash)
